...
def parse_args():
    parser = argparse.ArgumentParser(description="publish some message to the network server")
    ...
    parser.add_argument('--message', '-m', nargs='+', help='the messages to publish')
    parser.add_argument('--qos', '-q', type=int, choices=[0, 1, 2], default=0, help='qos of messages')
    parser.add_argument('--retain', '-r', action='store_true', help='retain of messages')
...
def client_publish():
    ...
    print("Trying to connect {0}:{1} with client_id {2}".format(host, port, client_id))
