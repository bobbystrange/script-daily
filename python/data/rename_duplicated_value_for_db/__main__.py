...
class Main:
    ...
    def __init__(self, host, user, password, rename_strategy,
                 primary_key='id', has_effect=False, use_timestamp=False, time_name=None):
        self.db = None
        self.host = host
        self.user = user
    ...
    def rename(self, schema, column):
        ...
        if len(rows1) == 0:
            print(f"no duplicated columns in the schema '{schema}' on column '{column}'")
            return
        ...
        if self.use_timestamp:
            cursor2.execute(f'select {self.primary_key}, {column}, {self.time_name} '
        ...
        for kv in column_dict.items():
            ...
            for pk in pks:
                ...
                new_name = self.rename_strategy.get_new_name(col, primary_key, timestamp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', '--user', required=False, default='root', type=str)
    parser.add_argument('-p', '--password', required=True, type=str)
    parser.add_argument('-s', '--schema', required=True, type=str, help='database.table')
    ...
    with Main(host=args.host, user=args.user, password=args.password,
              has_effect=args.effect, rename_strategy=strategy_constructor()) as main:
        schema_name = args.schema
        column_name = args.column
        try:
            result = main.has_unique_column(schema=schema_name, column=column_name)
            if result is None:
                print(f'the column {column_name} in schema {schema_name} does not exist', file=sys.stderr)
                exit(0)
            elif result is True:
                print(f'the column {column_name} in schema {schema_name} has a unique index so I gonna skip it')
                exit(0)
